"""Control-code neutralization helpers for every output sink.

The single owner of the control-byte delete maps. Lives in ``common/`` so that
non-output consumers that neutralize their own sinks - ``common/loader/`` and
``exporters/``, both forbidden by the one-way-dependency rail from importing
``outputs/`` - can reach it. ``outputs/_sanitize.py`` re-exports these two
functions for the output surface.
"""
